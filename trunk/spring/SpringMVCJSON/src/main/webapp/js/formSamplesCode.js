...
function documentReady()
{
	...
	teamOneItems = $('td.#team0 input');
	teamOneItems.click(function(eventObj){selectPlayer({'teamId':0},eventObj);});

	teamTwoItems = $('td.#team1 input');
	teamTwoItems.click(function(eventObj){selectPlayer({'teamId':1},eventObj);});

	teamThreeItems = $('td.#team2 input');
	teamThreeItems.click(function(eventObj){selectPlayer({'teamId':2},eventObj);});
	
	teamFourItems = $('td.#team3 input');
	teamFourItems.click(function(eventObj){selectPlayer({'teamId':3},eventObj);});
...
function selectPlayer(eventData, eventObj)
{
 ...
 //alert("team id "+teams[selectedTeam].teamName+" player "+players[selectedPlayer].name+" parent "+$(eventObj.target).parent().attr('id'));
  ...
  for (i= 0;i<teams.length;i++)
  {
      ...
      $(labelFinder).removeClass('greyOut');
