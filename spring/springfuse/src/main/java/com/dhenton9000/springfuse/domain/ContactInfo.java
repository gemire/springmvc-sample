...
import static com.dhenton9000.springfuse.util.ResourcesUtil.DATE_FORMAT_PATTERN;
...
@JpaUnique
public class ContactInfo implements Identifiable<Integer>, Serializable, Copyable<ContactInfo> {
     ...
     */
    @Override
    public void copyTo(ContactInfo contactInfo) {
        contactInfo.setId(getId());
        //contactInfo.setAccountId(getAccountId());
        contactInfo.setCivility(getCivility());
        contactInfo.setLastName(getLastName());
        contactInfo.setFirstName(getFirstName());
        contactInfo.setBirthDate(getBirthDate());
        contactInfo.setOtherDate(getOtherDate());
        if (getParent() != null) {
            contactInfo.setParent(new Account(getParent().getPrimaryKey()));
        }
    }
}