...
@Entity
@Table(name = "ACCOUNT")
@NamedQuery(name = "Account.selectAll", query = "from com.dhenton9000.springfuse.domain.Account as account where 1 = 1", hints = {
...
public class Account implements Identifiable<String>, Serializable, Copyable<Account> {
    ...
    public void copyTo(Account account) {
        ...
        if (getHomeAddress() != null) {
            account.setHomeAddress(new Address(getHomeAddress().getPrimaryKey()));
        }
        if (getContactInformation() != null) {
            account.setContactInformation(new ContactInfo(getContactInformation().getPrimaryKey()));
        }
        if (getBooks() != null) {
            List<Book> books = new ArrayList<Book>();
            for (Book book : getBooks()) {
                books.add(new Book(book.getPrimaryKey()));
            }
            setBooks(books);
        }
        if (getDocuments() != null) {
            ...
            for (Document document : getDocuments()) {
                documents.add(new Document(document.getPrimaryKey()));
            }
