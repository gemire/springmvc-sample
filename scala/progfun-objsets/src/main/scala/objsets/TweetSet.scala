...
abstract class TweetSet {
  ...
  /**
   * walk componentSet accumulating the results in newSet
   * the function is the filter
   */
  private def createUnion(componentSet: TweetSet, newSet: TweetSet, f: Tweet => Boolean): TweetSet = {
    if (componentSet.isEmpty) {
      newSet
    } else {
      if (f(componentSet.head)) {
        createUnion(componentSet.tail, newSet.incl(new Tweet(componentSet.head.user, componentSet.head.text, componentSet.head.retweets)), f)
      } else {
        createUnion(componentSet.tail, newSet, f)
      }
    }
  }
